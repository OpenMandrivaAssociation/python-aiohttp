...
# disable tests on abf as many require network interfaces unavailable in mock
%bcond tests 0
...
URL:		https://aio-libs.org/projects/aiohttp
...
k="${k-}${k+ and }not test_read_decode_compressed_exceeds_max_size"
k="${k-}${k+ and }not test_invalid_linebreak[pyloop]"
k="${k-}${k+ and }not test_invalid_character[pyloop]"
k="${k-}${k+ and }not test_aiohttp_plugin_async_gen_fixture"
k="${k-}${k+ and }not test_finalized"
k="${k-}${k+ and }not test_c_parser_loaded"
...
pytest -Wdefault ${ignore-} -k "${k-}" -rs tests/
%endif
